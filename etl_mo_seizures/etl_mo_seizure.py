...
class MoSeizureETL:
    ...
    def process_record_worker(self, seizure_record: Dict, chunk_range: str, chunk_state: Dict):
        ...
        try:
            ...
            with self.stats_lock:
                ...
                # (e.g. before insert_seizure()'s own try block is ever
                # reached) are no longer invisible in the bottom-line result.
                self.stats['errors'].append(
                    f"Seizure mo_seizure_id={seizure_record.get('MO_SEIZURE_ID', 'unknown')}: {e}"
                )
    ...
    def run(self):
        ...
        try:
            ...
            # OTHER failure (connection errors, transform errors, etc.) has no
            # retry path and means a real record was permanently dropped; such
            # a run must not be reported as a clean success.
            fk_retry_queued = self.stats['total_seizures_failed_crime_id']
            unhandled_failures = self.stats['total_seizures_failed'] - fk_retry_queued
            ...
            if unhandled_failures > 0:
                logger.error(
                    ...
                    f"(total failed={self.stats['total_seizures_failed']}, "
